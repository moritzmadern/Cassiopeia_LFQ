...
<< import default parameters, echo = FALSE, message = FALSE, warning = FALSE>>=

source("Cassiopeia_LFQ_default_parameters.R")
...
if (!is.null(pairwise_comp)){
  ...
  for (l in 1:length(pairwise_comp)){
    ...
    if (ncol(relevant_df_imp) == 2){
      writeLines("There is only one sample per group, hence limma is not possible.")
      writeLines("MA plot comparing two samples")
      df_plot_l <- data.frame(`Nice names`= df$`Nice names`, logFC = c(relevant_df_imp[,2] - relevant_df_imp[,1]), AveExpr = rowMeans(relevant_df_imp), check.names=FALSE)
    ...
    if (is.null(batch)){
      fit <- lmFit(relevant_df_imp, design)
      ...
      fit <- lmFit(relevant_df_imp, design, block= batch_variable, correlation = dupcor$consensus)
      fit_ebayes <- eBayes(fit, trend = trend_limma)
      LIMMAresults <- topTable(fit_ebayes, number=Inf, coef= group2, adjust="BH", sort.by="none")     
    }
    cat("\n")
...
if (export_matrix | export_amica){
  ...
  if ("con_table.txt" %in% list.files()){
    con_table <- read.delim(file="con_table.txt", header=TRUE, stringsAsFactors = FALSE)
