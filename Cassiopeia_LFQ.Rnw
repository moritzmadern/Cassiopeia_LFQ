...
\noindent Note: The GSEA uses the limma t-statistics as ranks, and gene names as identifiers. If multiple gene names are present, the first entry is used. "Upgregulated" in GSEA means that genes from the repective gene set have predominantly positive log2 fold changes, thus NES \textgreater 0. "Downregulated" on the other hand means that genes have predominantly negative log2 fold changes, thus NES \textless 0. \newline
