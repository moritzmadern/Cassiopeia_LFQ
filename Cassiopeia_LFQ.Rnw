...
\vspace{0.5cm} 
\noindent Note: Proteins in above volcano and MA plots were annotated via "Nice names", which correspond to gene name entries (column: "Gene names") reduced to just the first entry if multiple entries are separated by ";". \newline \newline
\noindent Note: The GSEA uses limma t-statistics as ranks, and gene names as identifiers.If multiple gene names are present, the first entry is used. "Upgregulated" in GSEA means that genes have predominantly positive log2 fold changes, thus NES \textgreater 0. "Downregulated" on the other hand means that genes have predominantly negative log2 fold changes, thus NES \textless 0. \newline
